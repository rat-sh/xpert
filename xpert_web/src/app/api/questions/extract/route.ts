...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!start.ok || !uploadUrl) return responseError('Could not upload the file for extraction. Please try again.', 502);
    ...
    if (!uploaded.ok) return responseError('The file upload was incomplete. Please try a smaller or clearer file.', 502);
    const uploadData = await uploaded.json() as { file?: { uri?: string; mimeType?: string; state?: string } };
    const remoteFile = uploadData.file;
    ...
    const generated = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${apiKey}`, {
    ...
    if (!generated.ok) return responseError('Question extraction failed. Try a clearer file or add questions manually.', 502);
